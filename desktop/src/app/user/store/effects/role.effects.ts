...
export class RoleEffects {
  ...
  // Listen for the 'ROLELIST_LOAD' action
  @Effect() roleListAction$ = this.action$
      .ofType(RoleActions.ROLELIST_LOAD)
  ...
  // Listen for the 'ROLE_LOAD' action
  @Effect() roleLoadAction$ = this.action$
      .ofType(RoleActions.ROLE_LOAD)
  ...
  // Listen for the 'ROLE_CREATE' action
  @Effect() roleCreateAction$ = this.action$
      .ofType(RoleActions.ROLE_CREATE)
      .map<Action, any>(toPayload)
  ...
  // Listen for the 'ROLE_UPDATE' action
  @Effect() roleUpdateAction$ = this.action$
      .ofType(RoleActions.ROLE_UPDATE)
      .map<Action, any>(toPayload)
    ...
    // Listen for the 'ROLE_REMOVE' action
    @Effect() roleRemoveAction$ = this.action$
        .ofType(RoleActions.ROLE_REMOVE)
        .map<Action, any>(toPayload)
